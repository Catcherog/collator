// TEMP: 妆造师资源数据一次性导入(从电子表格到多维表) | 2026-06-25 | 2026-06-28
const fs = require('fs');
...
const TABLE_ID = 'tblYwsDtJ2CHCA5T'; // 妆造师资源子表

// 字段映射配置（使用实际的字段名）
const FIELD_MAPPING = {
  target: ['姓名', '妆造报价'] // 暂时不导入档期（因为源数据多为文本描述）
};
...
async function main() {
  console.log('🚀 开始导入化妆师数据到中台...\n');

  try {
    // 读取源数据
    console.log('📖 读取源电子表格数据...');
    const readCmd = `lark-cli sheets +read --url "${SPREADSHEET_URL}" --sheet-id "08d824" --range "A2:G196" --as user`;
    ...
    for (let i = 0; i < batches; i++) {
      ...
      const jsonFile = `./batch_makeup_${i}.json`;
