// TEMP: 影棚/场地资源数据一次性导入(从电子表格到多维表) | 2026-06-25 | 2026-06-28
const fs = require('fs');
...
const TABLE_ID = 'tblabUs5800yEwaG'; // 场地资源子表

// 字段映射配置
const FIELD_MAPPING = {
  target: ['场地名称'] // 只映射场地名称（其他字段格式复杂）
};

function transformRow(row) {
  const [name] = row; // 只提取场地名称
...
async function main() {
  console.log('🚀 开始导入影棚数据到中台...\n');

  try {
    // 读取源数据
    console.log('📖 读取源电子表格数据...');
    const readCmd = `lark-cli sheets +read --url "${SPREADSHEET_URL}" --sheet-id "NIvi5C" --range "A2:U200" --as user`;
    ...
    // 转换数据
    console.log('🔄 转换数据格式...');
    const transformedData = rawData.map(transformRow).filter(row => row[0]); // 过滤掉没有场地名称的空行
    ...
    for (let i = 0; i < batches; i++) {
      ...
      const jsonFile = `./batch_studio_${i}.json`;
