...
Phase 2C（Immutable CleaningPipeline 已实现并验收通过，准备 Gate C 数据质量验收）

- 状态：DONE（Phase 2C 代码与测试已完成）
...
V1 Core Service 验收 / Gate C 数据质量验收

## In Progress

- Gate C 数据质量验收准备（50 条评测集与 CleaningPipeline 跑通）

## Recently Completed

- **Phase 0**：基线盘点。解压并阅读 `Collator_Dify_飞书_Trae实施协作包_v1.0` 全部手册与模板；阅读 `.trae/Knowledge/` 与 `.trae/rules/`；完成项目目录、Schema、核心模块、脚本、测试入口盘点；运行现有测试并记录真实结果；扫描硬编码资源 ID 与敏感信息；生成 `docs/BASELINE_REPORT.md`、`docs/PROJECT_STATE.md`、`docs/DECISIONS.md`、`docs/ACCEPTANCE_REPORT.md`；初始化 Git 仓库并提交 Phase 0 基线 Commit。
...
1. 进入固定评测集（Phase 2F）和 Gate C 数据质量验收。
2. 配置 Dify 环境与凭据（DEBT-001），解锁真实 LLM 回调链路。
3. 配置飞书测试 Base 凭据与表结构（DEBT-002），解锁 Gate D 飞书集成验收。
4. 后续按 Phase 推进流程进入 Phase 3（飞书集成）、Phase 5（部署）、Phase 6（展示证据）。

## Roadmap

### Phase 2F: 固定评测集与 Gate C 数据质量验收
- 里程碑：50 条评测集与 CleaningPipeline 跑通
- 依赖：Phase 2C 已完成
- 阻塞项：无代码阻塞，可立即开始
- 验收标准：Gate C 数据质量指标达标（总字段准确率≥90%，必填字段召回率≥95%，枚举映射精确率≥95%）
...
- 未配置 Dify 环境与凭据（`DIFY_BASE_URL`、`DIFY_WORKFLOW_API_KEY`、`DIFY_WORKFLOW_ID`）— 见 DEBT-001。
- 未配置飞书测试 Base 凭据与表结构（`FEISHU_APP_ID`、`FEISHU_APP_SECRET`、`FEISHU_BASE_APP_TOKEN` 及各表 ID）— 见 DEBT-002。

> 注：以上阻塞项均属于外部环境配置，不阻塞 V1 Core Service 代码合并，但阻塞 Gate D 飞书集成验收与端到端真实链路。
...
| C 数据质量 | IN_PROGRESS | Phase 2B 完成 Legacy Adapter；Phase 2C 完成 Immutable CleaningPipeline；50 条评测集与 Gate C 数据质量验收在 Phase 2F |
