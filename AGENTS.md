...
- 定义验收标准。
- 指出实现约束。
- 阅读 GitHub 中的代码、任务文件、提交和 PR。
- 根据 MVP 审查规则进行验收。
- 区分阻塞问题和非阻塞技术债。
- 输出标准化的任务包或修复包。

GPT 不得：
- 修改 GitHub 内容、本地文件或执行 Git 提交。
- 直接维护任务状态或将审查建议直接落库。
...
Trae 不得把 GPT 输出视为绝对正确。GPT 输出是任务输入，GitHub 中的最新代码、已接受决策和用户明确决定具有更高优先级。
